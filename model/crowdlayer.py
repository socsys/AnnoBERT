...
class CrowdsClassification(layers.Layer):
...
class CrowdsRegression(layers.Layer):
...
class MaskedMultiCrossEntropy(object):
	...
	def loss(self, y_true, y_pred):
		...
		vec = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=y_pred, labels=y_true, axis=1)
...
class MaskedMultiSequenceCrossEntropy(object):
	...
	def loss(self, y_true, y_pred):
		...
		y_true = tf.transpose(tf.one_hot(tf.cast(y_true, tf.int32), self.num_classes, axis=-1), [0,1,2])
		y_pred = tf.transpose(y_pred, [0,2,1])
		print(y_true.shape, y_pred.shape)
		print(mask_missings, mask_padding)
...
def CrowdLayer(train_crowd_matrix,seq_length=75):

	in_id = tf.keras.layers.Input(shape=(seq_length,), dtype='int64', name="input_ids")
	in_mask = tf.keras.layers.Input(shape=(seq_length,), dtype='int64', name="input_masks")
	bert_inputs = [in_id, in_mask]

	bert_model = TFBertModel.from_pretrained('bert-base-uncased')(bert_inputs)
	bert = bert_model[0][:, 0, :]
	flat = layers.Flatten()(bert)
	classifier = layers.Dense(2, activation='softmax')(flat)
	crowd = CrowdsClassification(2, train_crowd_matrix.shape[1], conn_type="MW")(classifier)
	model = Model(inputs=bert_inputs, outputs=crowd)

	optimizer = tf.keras.optimizers.SGD(learning_rate=1e-7,momentum=0.9)
	loss = MaskedMultiSequenceCrossEntropy(2).loss
...
def Pre_Crowdlayer(model,seq_length=75):
	...
	# Recompile the model
	pmodel = Model(inputs=bert_inputs, outputs=predictions)
	print('\nPrediction Model', pmodel.summary())

	optimizer = tf.keras.optimizers.SGD(learning_rate=1e-7,momentum=0.9)
	loss = MaskedMultiSequenceCrossEntropy(2).loss
	pmodel.compile(loss=loss, optimizer=optimizer, metrics=['acc'])

	return pmodel


def get_crowd_matrix(crowddata,train_text,train_y):

    num_anno = len(crowddata['worker_id'].unique())
    name_anno = crowddata['worker_id'].unique()
    annotators = {}
    for i in range(num_anno):
      annotators[name_anno[i]] = i+1

    # Initialize Crowd matrix
    # shape = (num_item, num_workers) (6124,6)
    # use -1 to indicate missing labels
    train_crowd_matrix = np.zeros((len(train_y), num_anno), dtype=int) - 1 # waseem

    # Create Crowd Matrix
    for indx in range(len(train_text)):
        item_id = train_text.iloc[indx]["item_id"] # text indx
        filt_crowddata = crowddata.loc[crowddata["item_id"] == item_id] # labels for text indx
        # print(item_id,filt_crowddata)
        for i in range(len(filt_crowddata)):
            tup = filt_crowddata.iloc[i] # each annotation line
            train_crowd_matrix[indx, annotators[tup["worker_id"]]-1] = int(tup["label"])

    print(np.sum(train_crowd_matrix == 0))
    print(np.sum(train_crowd_matrix == 1))
    print(np.sum(train_crowd_matrix == -1))

    return train_crowd_matrix
